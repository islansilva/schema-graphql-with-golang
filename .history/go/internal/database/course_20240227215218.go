...
func (c *Course) FindByCategoryID(categoryID string) (*Course, error) {
	rows, err := c.db.Query("SELECT id, name, description, categoryid FROM courses where categoryid = $1", categoryID).Scan(&id, &name, &description, &categoryID); err != nil {
		return nil, err
	}
	if err!=nil {
		return nil, err
	...
	return &Course{ID: id, Name: name, Description: &description, CategoryID: categoryID}
